...
const TaskContainer: React.FC<TaskContainerProps> = ({ tabId, tasks, setTasks, SetModalOpen }) => {
    ...
    const handleChangeProgress = (id: string) => {
        const newTasks = [...tasks];
        const taskIndex = newTasks.findIndex(task => task.id === id);
    ...
    useEffect(() => {
        ...
        switch (tabId) {
            ...
            case 2:
                ...
                filteredTasks = tasks.filter(task => task.date === formattedDate);
                break;
            case 3:
        ...
        const uncompletedTasks = filteredTasks.filter(task => !task.isComplete);
        const totalTasks = filteredTasks.length;

        const percentComplete = totalTasks > 0 ? (completedTasks.length / totalTasks) * 100 : 0;
        const percentUncomplete = totalTasks > 0 ? (uncompletedTasks.length / totalTasks) * 100 : 0;
            ...
            <div className={display !== "row" ? "task-list" : "task-list-row"}>
                {displayTask(tasks).map((task, index) => (
                    <Task
                        ...
                        changeProgress={() => handleChangeProgress(task.id)}
